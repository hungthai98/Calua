...
if ( !function_exists( 'trx_addons_cpt_layouts_show_layout' ) ) {
	...
	function trx_addons_cpt_layouts_show_layout($id) {	
		...
		if (!empty($layout)) {
			...
			echo strpos($layout->post_content, '[vc_row')!==false
				? do_shortcode($layout->post_content)
				: apply_filters('trx_addons_filter_sc_layout_content', $layout->post_content, $layout->ID);
	...
	function trx_addons_cpt_layouts_sc_wrap($output, $sc, $atts, $content) {
		...
		return !empty($tag)
					? '<div class="sc_layouts_item'
						. (strpos($tag, 'sc_layouts_menu_mobile_button') && strpos($tag, 'without_menu')!==false
